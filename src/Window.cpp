    ...
    for (int x = 0; x < 144; x++) {
      for (int y = 0; y < 160; y++) {
        ...
        sf::Color pixel_color =
            getPixelColor(getTileData(gb, tile_index), x % 8, y % 8);
...
void closeWindow() { ImGui::SFML::Shutdown(); }
...
void drawMemoryViewer() {
  if (showMemoryViewer) {
    ...
    for (int i = 0; i < 0x1000; i += 16) {
      ...
      for (int j = 0; j < 16; ++j) {
        ImGui::SameLine();
        // Check if the current memory cell is the same as the program counter
        // (PC)
        if ((i + j) == gb.PC) {
      ...
      for (int j = 0; j < 16; ++j) {
        ImGui::SameLine();
        char asciiChar = (gb.ROM[i + j] >= 32 && gb.ROM[i + j] <= 126)
                             ? static_cast<char>(gb.ROM[i + j])
                             : '.';
...
void drawCPUInfo() {
  if (showCPUInfo) {
    ...
    ImGui::Text("A: %02X   B: %02X   C: %02X   D: %02X", gb.A, gb.B, gb.C,
                gb.D);
    ...
    ImGui::Text("Transfer enabled: %d, Clock doubled : %0d, Clock: %s",
                trans_enabled, clock_double,
                which_clock == 0 ? "Slave" : "Master");
...
/*void loadFile()
{
        ...
        if (result == NFD_OKAY) {
                ...
                std::ifstream romFile(outPath, std::ios::binary |
std::ios::ate);
                ...
                if (romFile.is_open()) {
                        ...
                        // Use std::vector for dynamic memory allocation and
automatic cleanup std::vector<char> buffer(fileSize);

                        if (romFile.read(reinterpret_cast<char*>(buffer.data()),
fileSize)) {
                                // Assuming GB is an instance of your GameBoy
class gb.loadRom(buffer.data(), fileSize); fmt::println("ROM loaded
successfully."); gb.status = PAUSED;
                        }
                        else {
        ...
        // NFD automatically allocates memory for outPath, so there's no need to
free it
}*/
