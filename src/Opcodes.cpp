...
void GameBoy::ld_hl_sp_i8() {
  PC++;
  int8_t immediate_value = (int8_t)getBus(PC);
  uint16_t original_sp = SP; // Use the original SP value for flag calculations

  // Perform the addition and store the result in HL
  setHL(original_sp + immediate_value);

  // Zero and Negative flags are always reset for this instruction.
  ZeroFlag = 0;
  NegativeFlag = 0;

  // The flags for this instruction are calculated based on the addition
  // of the lower byte of the stack pointer and the immediate value.
  // For flag purposes, the immediate value is treated as an unsigned byte.
  uint8_t unsigned_immediate = (uint8_t)immediate_value;

  // Half-carry is set if there is a carry from bit 3.
  HalfCarry = ((original_sp & 0xF) + (unsigned_immediate & 0xF)) > 0xF;

  // Carry is set if there is a carry from bit 7.
  Carry = ((original_sp & 0xFF) + unsigned_immediate) > 0xFF;

  // Increment cycles and program counter
  add_m_cycles(3);
  add_t_cycles(12);
  PC++;
}
...
void GameBoy::add_sp_i8() {
  PC++;
  int8_t value = (int8_t)getBus(PC);
  uint16_t orig_SP = SP;

  // The addition is performed as a 16-bit operation.
  SP += value;

  // Flags are determined by the lower byte of SP and the immediate value.
  // The immediate value is treated as unsigned for flag calculations.
  uint8_t unsigned_value = (uint8_t)value;
  uint8_t sp_low = (uint8_t)(orig_SP & 0xFF);

  // Zero and Negative flags are always reset.
  ZeroFlag = 0;
  NegativeFlag = 0;

  // Half-carry is set if there is a carry from bit 3.
  HalfCarry = ((sp_low & 0xF) + (unsigned_value & 0xF)) > 0xF;

  // Carry is set if there is a carry from bit 7.
  Carry = ((uint16_t)sp_low + unsigned_value) > 0xFF;
...
void GameBoy::adc_a_reg(uint8_t reg) {
  ...
  ZeroFlag = !(A);
  HalfCarry =
      (((original_val & 0xF) + (reg & 0xF) + (Carry & 0xF)) & 0x10) == 0x10;
...
void GameBoy::sbc_a_hl() {
  uint8_t value_from_hl = getBus(getHL());
  uint8_t original_a = A;
  uint8_t carry_val =
      Carry; // The value of the carry flag from the previous operation

  // Perform the subtraction: A = A - (value at HL) - Carry
  uint16_t result = original_a - value_from_hl - carry_val;
  A = (uint8_t)result;

  // Set Flags
  ZeroFlag = (A == 0);
  NegativeFlag = 1; // Negative flag is always set for subtraction

  // Half-carry is calculated on the lower nibble before the final subtraction
  HalfCarry = (original_a & 0xF) < ((value_from_hl & 0xF) + carry_val);

  // Carry is set if the result is negative (a borrow occurred)
...
void GameBoy::daa() {
  ...
  if (!NegativeFlag) { // after an addition, adjust if (half-)carry occurred or
