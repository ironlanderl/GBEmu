...
void GameBoy::add_m_cycles(uint8_t cycles_to_add) { m_cycles += cycles_to_add; }

void GameBoy::add_t_cycles(uint8_t cycles_to_add) { t_cycles += cycles_to_add; }

void GameBoy::handle_interrupts() {
  // Check for any pending interrupts.
  uint8_t pending = getIE() & getIF();

  if (halted && (pending & 0x1F) != 0) {
    halted = false;
  }
  ...
  if (pending & 0x01) // Priority 1: V-Blank
  {
    run_vblank();
  } else if (pending & 0x02) // Priority 2: LCD STAT
  {
    run_lcd();
  } else if (pending & 0x04) // Priority 3: Timer
  {
    run_timer();
  } else if (pending & 0x08) // Priority 4: Serial
  {
    run_serial();
  } else if (pending & 0x10) // Priority 5: Joypad
  {
    run_joypad();
  }
}
...
void GameBoy::run_tick() {
  ...
  if (getBus(0xFF44) >= 144) {
    // If we are in VBlank, set the VBlank interrupt
    writeBus(getIF() | 0x01, 0xFF0F);
  } else {
...
uint8_t GameBoy::getIE() { return getBus(0xFFFF); }

uint8_t GameBoy::getIF() { return getBus(0xFF0F); }

void GameBoy::increment_ly() {
  auto ly = getBus(0xFF44);
  ly = (ly + 1) % 154;
  writeBus(ly, 0xFF44);
}

bool GameBoy::isInsideInterval(uint16_t value, uint16_t bottom_bound,
...
uint8_t GameBoy::get_next_byte(uint16_t address) { return getBus(address + 1); }

uint16_t GameBoy::fuse_two_bytes(uint8_t a, uint8_t b) { return (a << 8) | b; }
...
uint16_t GameBoy::getHL() { return ((H << 8) | L); }
...
void GameBoy::run_lcd() {
  IME = false;

  writeBus(getIF() & ~0x02, 0xFF0F);
...
void GameBoy::run_timer() {}

void GameBoy::run_serial() {}

void GameBoy::run_joypad() {}
