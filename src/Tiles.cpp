...
/**
 * @brief Retrieves the tile data for a given tile index from the GameBoy's
 * memory.
 *
 * This function extracts 16 bytes of tile data starting from the specified tile
 * index. The tile data is fetched from the GameBoy's video memory starting at
 * address 0x8000.
 *
 * @param gb Reference to the GameBoy instance from which to retrieve the tile
 * data.
 * @param tile_index The index of the tile to retrieve. Each tile consists of 16
 * bytes.
 ...
 *
 * This function takes tile data in the form of a 16-byte array and draws an 8x8
 * tile on the specified SFML render window. Each byte in the tile data
 * represents two bits of color information for each pixel in the tile.
 *
 * @param renderer The SFML render window where the tile will be drawn.
 * @param tile_data A 16-byte array containing the tile data. Each pair of bytes
 * represents the color information for one row of the tile.
 ...
 * - 3: Black (0, 0, 0)
 */
void drawTile(sf::RenderWindow &renderer,
              const std::array<uint8_t, 16> &tile_data, int x_offset,
              int y_offset, int pixel_size) {
  ...
  for (int y = 0; y < 8; ++y) {
    ...
    for (int x = 0; x < 8; ++x) {
      ...
      // Determine the color of the pixel by combining the bits from the two
      // bytes
      ...
      pixel_rect.setPosition(
          sf::Vector2f(x_offset + x * pixel_size, y_offset + y * pixel_size));
 ...
 *
 * This function takes tile data in the form of a 16-byte array and returns the
 * color of the pixel at the specified (x, y) coordinates within the tile.
 *
 * @param tile_data A 16-byte array containing the tile data. Each pair of bytes
 * represents the color information for one row of the tile.
 ...
 * - 3: Black (0, 0, 0)
 */
sf::Color getPixelColor(const std::array<uint8_t, 16> &tile_data, int x,
                        int y) {
  // Each row is represented by two bytes in the tile data
  uint8_t bit_1 = tile_data[y * 2];
  uint8_t bit_2 = tile_data[y * 2 + 1];
